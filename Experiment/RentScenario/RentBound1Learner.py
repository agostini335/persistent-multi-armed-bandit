...
class RentPersistentSingleExpl(RentLearner):

    def __init__(self, n_arms, arms, tmax_a, tmax_s, t_global):
        super().__init__(n_arms,arms,"RentPersistentSingleExpl")
    ...
    def update_observations(self, pulled_arm, bucket):
        ...
        for arm in range(self.n_arms):
            ...
            for i in range(self.t_global):                
                ...
                num +=  x_i
                den +=  x_i + s_i
            c = np.sqrt((2*np.log(self.t))/(np.sum(self.hits_table)+np.sum(self.sfitto_table)))
            #print("num:"+str(num)+" den: "+str(den)+ " c: "+str(c))
            if den!=0:
                criterion = self.arms[arm].canone * ((num/den) + c)            
                self.bound1_criterion[arm]= criterion
