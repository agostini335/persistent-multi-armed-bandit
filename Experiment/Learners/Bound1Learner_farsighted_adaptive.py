...
class Bound1Learner_farsighted_adaptive(Learner):
    """
    class used to represent the Bound1 Learner farsighted: the learner is able to see the full bucket after
        the first zero.
    ....
    ...
    def __init__(self, n_arms, arms, tmax):
        super().__init__(n_arms,arms,"Bound1_farsighted_adp")
    ...
    def update_observations(self, pulled_arm, bucket):
        ...
        for arm in range(self.n_arms):
            # b is the oldest
            if(len(self.arms[arm].buckets)>0):
                b = self.arms[arm].buckets[0]
            ...
            for b in self.arms[arm].buckets:
                ...
                if b.values[m]>0:
                    ...
                    if(m<self.tmax-1):
                        ...
                        assert(self.tmax_seen == self.tmax)
                else:
                    #full update visite
        ...
        for arm in range(self.n_arms):
            criterion = 0
            for i in range(self.tmax_seen):
                x_i = c_i = 1
                if(self.visits_table[i][arm]>0):
                    x_i = self.hits_table[i][arm]/self.visits_table[i][arm]
                    c_i = np.sqrt( 2 * np.log( self.t*np.power(self.tmax_seen,1/4))/ self.visits_table[i][arm])                
